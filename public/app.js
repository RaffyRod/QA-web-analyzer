...
function formatCodeSnippet(html, maxLength = 200) {
  ...
  if (isLong) {
    ...
    return (
      ...
      '<code class="code-snippet" id="' +
      uniqueId +
      '-short">' +
      truncated +
      '</code>' +
      '<code class="code-snippet hidden" id="' +
      uniqueId +
      '-full">' +
      escaped +
      '</code>' +
    ...
    .map((img) => {
      ...
      if (options.checkFocusStates) {
        ...
        const focusStateInMissing =
          img.missingAttributes && img.missingAttributes.includes('focus-state');
        attributesHtml += `
        <div class="attr-item">
...
function displayLinks(links, options) {
  ...
  linksList.innerHTML = links
    .map((link) => {
      const missingAttrs =
        link.missingAttributes && link.missingAttributes.length > 0 ? link.missingAttributes : [];
      ...
      if (options.checkFocusStates) {
        ...
        const focusStateInMissing =
          link.missingAttributes && link.missingAttributes.includes('focus-state');
        attributesHtml += `
        <div class="attr-item">
          <span class="attr-name">Focus State:</span>
          <span class="attr-value ${hasFocusState && !focusStateInMissing ? 'present' : 'missing'}">${hasFocusState && !focusStateInMissing ? 'Present' : 'Missing'}</span>
...
function displayButtons(buttons, options) {
  ...
  buttonsList.innerHTML = buttons
    .map((btn) => {
      const missingAttrs =
        btn.missingAttributes && btn.missingAttributes.length > 0 ? btn.missingAttributes : [];
      ...
      if (options.checkFocusStates) {
        ...
        const focusStateInMissing =
          btn.missingAttributes && btn.missingAttributes.includes('focus-state');
        attributesHtml += `
        <div class="attr-item">
          <span class="attr-name">Focus State:</span>
          <span class="attr-value ${hasFocusState && !focusStateInMissing ? 'present' : 'missing'}">${hasFocusState && !focusStateInMissing ? 'Present' : 'Missing'}</span>
...
function displayInputs(inputs, options) {
  ...
  inputsList.innerHTML = inputs
    .map((input) => {
      const missingAttrs =
        input.missingAttributes && input.missingAttributes.length > 0
      ...
      if (options.checkFocusStates) {
        const focusStateInMissing =
          input.missingAttributes && input.missingAttributes.includes('focus-state');
        attributesHtml += `
        <div class="attr-item">
          <span class="attr-name">Focus State:</span>
...
function displayRoles(roles, options) {
  ...
  rolesList.innerHTML = roles
    .map((role) => {
      const missingAttrs =
        role.missingAttributes && role.missingAttributes.length > 0 ? role.missingAttributes : [];
